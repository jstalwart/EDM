...
ggplot(th, aes(x = round(temp/4)*4, y = round(hum/6)*6, fill = p)) + geom_tile() + geom_rug(mapping= aes(x =temp), sides = "b", inherit.aes = F) + geom_rug(mapping= aes(y =hum), sides = "l", inherit.aes = F) + scale_x_continuous(limits = c(0, 35))+ scale_y_continuous(limits = c(25, 95))+xlab('Temperatura')+ylab("Humedad") + labs(fill="Número de bicicletas")
```
